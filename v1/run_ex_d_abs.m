...
initCond = {0,[]}; % initCond = {u1(t0), u2(t0),...,un(t0)}, put [] on emty spaces for BVP's
finalCond = {0.11, []}; % For IVP finalCond={}. For BVP finalCond = {u1(L), u2(L),...,un(L)}, put [] on emty spaces  

%% Define the equation, its derivatives and an initial guess
f = @(t,x) [x(2),-(x(1)^2)-x(1)+abs(t-1/2)];
d_uf = @(t,x)[0, 1; -2*x(1)-1, 0];
